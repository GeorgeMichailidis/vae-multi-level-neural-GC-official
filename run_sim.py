...
def main():
    ...
    if not len(args.output_dir): ## no override
        ...
        setattr(args, 'output_dir', os.path.join('output_sim',f'{args.ds_str}_seed{args.seed}-{suffix}-{args.train_size}'))
...
def _check_configs(configs):
    ...
    supported_models = ['simMultiSubVAE']
    assert configs['network_params']['model_type'] in supported_models
